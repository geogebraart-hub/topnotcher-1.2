...
function DeckDetail({deck,questions,questionStats,flashcards,onGenerateFlashcards,onDeleteFlashcard,onBack,onAdd,onAI,onEdit,onDelete,onStudy,onStudyFlashcards,onShare}) {
  ...
  return (
    ...
    {activeTab==="questions"&&<section className="panel question-bank"><div className="section-head"><div><h2>Questions</h2><span className="muted">{filteredQuestions.length} of {questions.length} · click a question to view it</span></div><div className="question-bank-actions"><label className="deck-search-box"><Search size={16}/><input value={deckSearch} onChange={e=>setDeckSearch(e.target.value)} placeholder="Search questions or flashcards..." aria-label="Search this deck"/></label><button className="secondary-btn compact" onClick={()=>downloadDeckQuestionsPdf(deck,questions)} disabled={!questions.length}><FileDown size={16}/> Download Questions PDF</button><button className="secondary-btn compact question-scanner-btn" onClick={runQuestionScanner} disabled={questions.length<2}><Search size={16}/> {scanRunning?"Scanning…":"Scan for Similar Questions"}</button></div></div>
